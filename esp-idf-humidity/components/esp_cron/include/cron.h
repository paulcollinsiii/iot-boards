 ...
 *  - id: identifier in the module for this cron job, this allows to unschedule
 * similar tasks
 *  - next execution: this information holds the time when it will run next, is
 * managed by the cron module
...
/*
 *  SUMARY: Allocates a cron job on the heap with supplied parameters, if cron
 * is not running, it will call cron_start()
 *
...
cron_job *cron_job_create(const char *schedule, cron_job_callback callback,
                          void *data);
 ...
 *
 *  PARAMS: removes all cron_jobs (no deallocation is performed by the call,
 * memory must be handled out of this module)
 *
...
/*
 *  SUMMARY: Starts the schedule module (creates a new task on the operating
 * system)
 *
 *  RETURNS:   cron_job_errors enum constant  for error checking
 */
...
/*
 *  SUMMARY: Stops the schedule module (deletes the cron task on the operating
 * system)
 *
 *  RETURNS:   cron_job_errors enum constant  for error checking
 */
 ...
 *
 *  PARAMS: cron_job to be scheduled (no allocation is performed by the call,
 * memory must be handled out of this module)
 *
 *  RETURNS:   cron_job_errors enum constant  for error checking
 */
 ...
 *
 *  PARAMS: cron_job to be removed (no deallocation is performed by the call,
 * memory must be handled out of this module)
 *
 *  RETURNS:   cron_job_errors enum constant  for error checking
 */
...
/*
 *  SUMARY: TASK SCHEDULER WITH DELAYS AND CALL TO CALLBACKS DO NOT RUN THIS
 * TAST
 *
 *  PARAMS: ARGS MUST BE NULL IN REGULAR USE, IF YOU WANT TO RUN JUST ONCE ARGS
 * MUST BE A CHAR POINTER TO "R1" LITERAL
 *
 *  RETURNS: NO RETURN
 */
 ...
 *  RETURNS: NO RETURN
 */
enum cron_job_errors cron_job_load_expression(cron_job *job,
                                              const char *schedule);
/*
