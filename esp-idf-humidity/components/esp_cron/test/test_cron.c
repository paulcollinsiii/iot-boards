...
TEST_CASE(
    "**CRON_JOB - INFO -- INIT TEST, THIS INITIALIZES THE TIME DATA MAY LEAK "
    "SOME MEMORY",
    "[cron_job]") {
  struct timeval tv;
...
TEST_CASE("**CRON_JOB - cron_job_schedule and cron_job_remove IS IT WORKING? ",
          "[cron_job]") {
  reset_cron();
...
TEST_CASE(
    "**CRON_JOB - cron_job_schedule CRON PARSER AND SCHEDULER IS AS EXPECTED ",
    "[cron_job]") {
  reset_cron();
  time_t now;
  struct timeval tv;
  ...
  TEST_ASSERT_EQUAL_INT_MESSAGE(
      now + 1, job->next_execution,
      "SCHEDULE IS NOT FOR THE NEXT SECOND AS IT SHOULD BE");
  ans = cron_job_destroy(job);
...
TEST_CASE(
    "**CRON_JOB - cron_schedule_task TEST IF THE SCHEDULER RUNS ONCE AND WITH "
    "EXPECTED RESULTS",
    "[cron_job]") {
  reset_cron();
  time_t begin = 1530000000;
  const int seconds_to_run =
      10 * 2;  // whatever you want to run must be twice because one for the
               // delay and one for the callback :P
  cron_job *jobs[2];
  struct timeval tv;
  tv.tv_sec = begin;  // SOMEWHERE IN JUNE 2018
  settimeofday(&tv, NULL);
  jobs[0] =
      cron_job_create("* * * * * *", test_cron_job_sample_callback, (void *)0);
  jobs[1] = cron_job_create("*/5 * * * * *", test_cron_job_sample_callback,
                            (void *)10000);
  ...
  TEST_ASSERT_MESSAGE(
      (int)jobs[0]->data >= 5 * 8,
      "Unexpected delay");  // IT DEPENDS ON TIME MOST OF THE TIME WILL DO 45,
                            // SOME 40 AND OTHERS 50
  TEST_ASSERT_MESSAGE(
      (int)jobs[1]->data >= 10005,
      "Unexpected delay");  // SOMETIMES IT RUNS TWICE BECAUSE OF BEGIN TIME
  cron_job_destroy(jobs[0]);
...
TEST_CASE(
    "**CRON_JOB - cron_schedule_task TEST IF THE SCHEDULER RUNS WHILE STARTED",
    "[cron_job]") {
  reset_cron();
  time_t begin = 1530000000;
  cron_job *job;
  ...
  TEST_ASSERT_EQUAL_INT_MESSAGE(
      Cron_ok, res,
      "Cron did not start");  // IT DEPENDS ON TIME MOST OF THE TIME WILL DO 45,
                              // SOME 40 AND OTHERS 50
  vTaskDelay(
      (6 * 1000) /
      portTICK_PERIOD_MS);  // WAIT FOR THE TASK TO RUN - WE DO 6 SECONDS BUT
                            // CHECK AGAINST 5 ASSUMING SOME ERRORS ON THE DELAY
                            // BETWEEN A TASK CREATION AND STUFF
  cron_stop();
  TEST_ASSERT_MESSAGE((int)job->data > 5 * 5,
                      "Unexpected delay");  // IT DEPENDS ON TIME
  cron_job_destroy(job);
  // cron_job_clear_all();
}

TEST_CASE(
    "**CRON_JOB - cron_job_create() should return a job with the data 1 2 3 * "
    "* *",
    "[cron_job]") {
  reset_cron();
  ...
  TEST_ASSERT_EQUAL_INT_MESSAGE(strncmp(job->data, "the data", 8), 0,
                                "the job data is not 1 2 3 * * *");
  vTaskDelay((1 * 1000) / portTICK_PERIOD_MS);
  cron_job_clear_all();
}

TEST_CASE("**CRON_JOB - cron_job_create() NULL if the schedule is wrong",
          "[cron_job]") {
  reset_cron();
  cron_job *job = cron_job_create("1 2 3", NULL, (char *)"the data");
...
TEST_CASE("**CRON_JOB - cron_job_destroy() return 0 if job is a cron_job",
          "[cron_job]") {
  reset_cron();
...
TEST_CASE(
    "**CRON_JOB - cron_job_destroy() return 0 if after 2 jobs creation but "
    "just one is destroyed",
    "[cron_job]") {
  reset_cron();
...
TEST_CASE(
    "**CRON_JOB - cron_job_destroy() return 0 if after 4 jobs creation but "
    "just one is destroyed",
    "[cron_job]") {
  reset_cron();
  (void)cron_job_create("1 2 3 * * *", NULL, (char *)"the data");
...
TEST_CASE("**CRON_JOB - cron_job_destroy() return -1 if job is null",
          "[cron_job]") {
  reset_cron();
...
TEST_CASE(
    "**CRON_JOB - cron_job_clear_all() anfter execute cron_job_clear_all the "
    "job is null",
    "[cron_job]") {
  reset_cron();
  (void)cron_job_create("1 2 3 * * *", NULL, (char *)"the data");
...
TEST_CASE("**CRON_JOB - cron_job_clear_all() called with an empty list",
          "[cron_job]") {
  reset_cron();
...
TEST_CASE("**CRON_JOB - cron_stop() return 0 if cron is started",
          "[cron_job]") {
  reset_cron();
  cron_start();
...
TEST_CASE(
    "**CRON_JOB - cron_stop() return -1 if cron is not running and method is "
    "called again",
    "[cron_job]") {
  reset_cron();
  (void)cron_stop();
  int res = cron_stop();
  TEST_ASSERT_EQUAL_INT_MESSAGE(res, Cron_is_stopped,
                                "return of cron_stop is not -1");
}

TEST_CASE("**CRON_JOB - cron_start() return -1 if cron is already started",
          "[cron_job]") {
  reset_cron();
  cron_start();
  (void)cron_job_create("1 2 3 * * *", NULL, (char *)"the data");
  int res = cron_start();
  TEST_ASSERT_EQUAL_INT_MESSAGE(res, Cron_not_stopped,
                                "return of cron_start is not -1");
  vTaskDelay((1 * 1000) / portTICK_PERIOD_MS);
  (void)cron_job_clear_all();
}
TEST_CASE("**CRON_JOB - cron_job_schedule() return -1 if job is null",
          "[cron_job]") {
  reset_cron();
...
TEST_CASE("**CRON_JOB - cron_job_unschedule() return -1 if job is null",
          "[cron_job]") {
  reset_cron();
...
TEST_CASE("**CRON_JOB - cron_job_unschedule() return 0 if job is ok",
          "[cron_job]") {
  reset_cron();
...
TEST_CASE(
    "**CRON_JOB - cron_job_load_expression() return 0 if job and schedule are "
    "correct",
    "[cron_job]") {
  reset_cron();
...
TEST_CASE(
    "**CRON_JOB - cron_job_load_expression() return -1 if schedule is null",
    "[cron_job]") {
  reset_cron();
  cron_job_list_init();  // CALL THIS ON ANY CREATE
  cron_job *job = calloc(sizeof(cron_job), 1);
...
TEST_CASE(
    "**CRON_JOB - cron_job_load_expression() return -2 if schedule is wrong 3 "
    "2 5",
    "[cron_job]") {
  reset_cron();
  cron_job_list_init();  // CALL THIS ON ANY CREATE
  ...
  int res = cron_job_load_expression(job, "3 2 5");
  cron_stop();
  TEST_ASSERT_EQUAL_INT_MESSAGE(res, Cron_parse_expresion_error,
                                "res is not -1");
}

TEST_CASE(
    "**CRON_JOB - cron_job_load_expression() return -2 if schedule is wrong "
    "*****",
    "[cron_job]") {
  reset_cron();
  cron_job_list_init();  // CALL THIS ON ANY CREATE
...
TEST_CASE("**CRON_JOB - cron_job_has_loaded() return 0 if job is scheduled",
          "[cron_job]") {
  reset_cron();
  cron_job *job = cron_job_create("3 2 5 * * *", NULL, (char *)"the data");
...
TEST_CASE("**CRON_JOB - cron_job_has_loaded() return -2 if job is scheduled",
          "[cron_job]") {
  reset_cron();
...
TEST_CASE(
    "**CRON_JOB - cron_job_has_loaded() return Cron_expresion_not_loaded if "
  ...
  job->load = NULL;
  int res = cron_job_has_loaded(job);
  TEST_ASSERT_EQUAL_INT_MESSAGE(res, Cron_expresion_not_loaded,
                                "res is not -2");
  cron_stop();
  free(job);
}
