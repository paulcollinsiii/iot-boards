...
kubectl apply -k ./kustomize/env/dev && \
  kubectl wait --for=condition=ready certificate -n cert-manager kaffi-homelab-ca && \
  kubectl get secret kaffi-homelab-ca -n cert-manager -o jsonpath="{.data.ca\.crt}" | base64 -d - >| ca.crt
